...
{
    /// <summary>
    /// Object pool implementation for large network buffers (64KB).
    /// Thread-safe pool using concurrent collections for high-performance scenarios.
    /// </summary>
    public sealed class LargeBufferPool : IObjectPool<PooledNetworkBuffer>, IDisposable
    {
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(LargeBufferPool));
        ...
        {
            var buffer = new PooledNetworkBuffer(65536) // 64KB for large buffers
            {
                PoolName = Name
            };
