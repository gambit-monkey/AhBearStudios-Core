...
namespace AhBearStudios.Core.Pooling.Pools
{
    ...
    public sealed class CompressionBufferPool : IObjectPool<PooledNetworkBuffer>, IDisposable
    {
        ...
        public CompressionBufferPool(PoolConfiguration configuration, IAdaptiveNetworkStrategyFactory strategyFactory)
        {
            _configuration = configuration ?? throw new ArgumentNullException(nameof(configuration));
            if (strategyFactory == null)
                throw new ArgumentNullException(nameof(strategyFactory));

            // Create strategy configuration for compression buffer pool
            var strategyConfig = new PoolingStrategyConfigBuilder()
                .WithName("CompressionBufferPool")
                .NetworkOptimized() // Use network optimization preset for compression scenarios
            ...
            _strategy = strategyFactory.CreateWithNetworkParameters(
                strategyConfig,
                spikeDetectionThreshold: 0.75, // Slightly more sensitive for compression workloads
                preemptiveAllocationRatio: 0.15, // Conservative preemptive allocation
                burstWindow: TimeSpan.FromSeconds(3), // Shorter burst window for compression
                maxBurstAllocations: 25); // Moderate burst handling
