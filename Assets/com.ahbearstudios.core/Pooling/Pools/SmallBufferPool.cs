...
{
    /// <summary>
    /// Object pool implementation for small network buffers (1KB).
    /// Thread-safe pool using concurrent collections for high-performance scenarios.
    /// </summary>
    public sealed class SmallBufferPool : IObjectPool<PooledNetworkBuffer>, IDisposable
    {
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SmallBufferPool));
        ...
        /// <summary>
        /// Creates a new buffer with the appropriate size for small buffers.
        /// </summary>
        /// <returns>A new PooledNetworkBuffer</returns>
        private PooledNetworkBuffer CreateNewBuffer()
        {
            var buffer = new PooledNetworkBuffer(1024) // 1KB for small buffers
            {
                PoolName = Name
            };
