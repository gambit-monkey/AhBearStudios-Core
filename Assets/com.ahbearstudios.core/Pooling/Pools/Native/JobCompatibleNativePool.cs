...
{
    /// <summary>
    /// A high-performance, job-compatible native pool implementation designed for use with Unity's job system.
    /// Uses Unity Collections v2 and supports parallel read operations for better performance in multi-threaded environments.
    /// This pool is thread-safe for reading but requires synchronization for write operations.
    /// </summary>
    /// <typeparam name="T">The unmanaged type to pool</typeparam>
    public class JobCompatibleNativePool<T> : INativePool<T>, IDisposable where T : unmanaged
    {
        ...
        // Cache for tracking active/inactive status
        ...
        /// <summary>
        /// Creates a new JobCompatibleNativePool with default settings.
        /// </summary>
        public JobCompatibleNativePool() : this(16, Allocator.Persistent)
        {
        }

        /// <summary>
        /// Creates a new JobCompatibleNativePool with specified capacity and allocator.
        /// </summary>
        ...
        public JobCompatibleNativePool(int initialCapacity, Allocator allocator, T defaultValue = default,
            string poolName = null)
        {
            // Store parameters
            Allocator = allocator;
            _defaultValue = defaultValue;
            PoolName = poolName ?? $"JobCompatibleNativePool<{typeof(T).Name}>";

            // Create configuration
            _config = new JobCompatiblePoolConfig(initialCapacity, allocator)
            {
                ThreadingMode = PoolThreadingMode.JobCompatible,
                UseSafetyChecks = true // Enable safety checks for job compatibility
            };
            ...
            if (_config.DetailedLogging && _logger != null)
            {
                _logger.LogInfoInstance(
                    $"JobCompatibleNativePool created: {PoolName} (Type: {typeof(T).Name}, Initial capacity: {initialCapacity})");
            }
        }
        ...
        /// Uses the registry-based handle for job compatibility.
        /// </summary>
