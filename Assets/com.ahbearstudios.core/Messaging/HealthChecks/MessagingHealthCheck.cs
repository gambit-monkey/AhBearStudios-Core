    ...
    public sealed class MessagingHealthCheck : IHealthCheck
    {
        ...
        public async UniTask<HealthCheckResult> CheckHealthAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // Execute health checks (circuit breaker functionality not available in current IHealthCheckService)
                // Check circuit breaker state if enabled
                if (_options.UseCircuitBreaker && _healthCheckService != null)
                {
                    var operationName = new FixedString64Bytes($"Messaging.{Name}");
                    var circuitBreakerState = _healthCheckService.GetCircuitBreakerState(operationName);
                    
                    if (circuitBreakerState == CircuitBreakerState.Open)
                    {
                        return CreateCircuitBreakerFallbackResult(stopwatch.Elapsed, data);
                    }
                }
            ...
            {
                stopwatch.Stop();
                _logger.LogException($"Messaging health check '{Name}' failed with unexpected error", ex);
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                stopwatch.Stop();
                _logger.LogException($"Messaging health check '{Name}' execution failed", ex);
                throw;
            }
        }
        ...
        private async UniTask<MessagingTestResult> TestPublisherFunctionality(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await publisher.PublishAsync(testMessage, timeoutCts.Token);
        ...
        private async UniTask<MessagingTestResult> TestMessageRoundTrip(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using var subscription = subscriber.Subscribe(messageHandler);
                ...
                var messageTask = messageReceivedTask.Task.AsUniTask();
                var receivedMessage = await messageTask.Timeout(roundTripTimeout);
        ...
        {
            var successfulChecks = healthChecks.AsValueEnumerable().Where(c => c.Success).Count();
            var totalChecks = healthChecks.Count;
            var avgDuration = totalChecks > 0 
                ? TimeSpan.FromTicks(healthChecks.AsValueEnumerable().Select(c => c.Duration.Ticks).Sum() / totalChecks)
                : TimeSpan.Zero;
            ...
            Dictionary<string, object> data)
        {
            return status switch
            {
                ...
                _ => HealthCheckResult.Unhealthy(Name.ToString(), "Unknown message bus health status", duration, data)
            };
        }
        ...
        private HealthCheckResult CreateUnhealthyResult(
            ...
            List<(string Name, bool Success, TimeSpan Duration, string Details)> healthChecks)
        {
            data["FailedChecks"] = healthChecks.AsValueEnumerable().Where(c => !c.Success).ToList();
            return HealthCheckResult.Unhealthy(Name.ToString(), reason, duration, data);
        }
        ...
        public void Dispose()
        {
            ...
            {
                _logger.LogException("Error disposing MessagingHealthCheck", ex);
            }
        }
