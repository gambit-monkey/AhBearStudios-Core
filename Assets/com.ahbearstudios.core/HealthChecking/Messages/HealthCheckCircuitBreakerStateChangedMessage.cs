...
namespace AhBearStudios.Core.HealthChecking.Messages
{
    ...
    public readonly record struct HealthCheckCircuitBreakerStateChangedMessage : IMessage
    {
            ...
            Guid correlationId = default)
        {
            ...
            var messageId = DeterministicIdGenerator.GenerateMessageId("HealthCheckCircuitBreakerStateChangedMessage", sourceString, correlationId: null);
            var finalCorrelationId = correlationId == default 
                ? DeterministicIdGenerator.GenerateCorrelationId("CircuitBreakerChange", $"{circuitBreakerName}-{oldState}-{newState}")
                : correlationId;
        ...
        {
            var reasonText = Reason.IsEmpty ? "None" : Reason.ToString();
            return $"CircuitBreaker '{CircuitBreakerName}': {OldState} -> {NewState} (Failures: {ConsecutiveFailures}, Reason: {reasonText})";
        }
    }
}