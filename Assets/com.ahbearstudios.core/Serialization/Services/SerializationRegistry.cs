...
/// <summary>
    ...
    public class SerializationRegistry : ISerializationRegistry
    {
        ...
        public SerializationRegistry(ILoggingService logger)
        {
            ...
            _logger.LogInfo("SerializationRegistry initialized", correlationId: correlationId, sourceContext: null, properties: null);
        }
        ...
        public void RegisterType(Type type, TypeDescriptor descriptor)
        {
            ...
            lock (_registrationLock)
            {
                ...
                {
                    _logger.LogInfo($"Registered type {type.FullName} with version {descriptor.Version}", correlationId: correlationId, sourceContext: null, properties: null);
                }
                else
                {
                    _registeredTypes[type] = descriptor;
                    _logger.LogInfo($"Updated registration for type {type.FullName}", correlationId: correlationId, sourceContext: null, properties: null);
                }
            }
        }
        ...
        public bool UnregisterType(Type type)
        {
            ...
            if (result)
            {
                _logger.LogInfo($"Unregistered type {type.FullName}", correlationId: correlationId, sourceContext: null, properties: null);
            }
        ...
        public void Clear()
        {
            ...
            _logger.LogInfo($"Cleared {count} registered types", correlationId: correlationId, sourceContext: null, properties: null);
        }
